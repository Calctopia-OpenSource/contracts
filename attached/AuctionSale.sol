 ...
 */
contract AuctionSale is Ownable, Pausable {
  using SafeMath for uint256;

  // The token being sold
  RAZToken public token;
  ...
  /**
   * Initialise an AuctionSale with a wallet
   * @param _wallet address of the wallet
   */
  function AuctionSale(address _wallet) public {
    require(_wallet != address(0));

    token = createTokenContract(_wallet);
    token.setSecondOwner(address(token));
    wallet = _wallet;
  }
   ...
   * @param _wallet address of the wallet
   */
  function createTokenContract(address _wallet) internal returns (RAZToken) {
	// example values
	// _exercisePrice = 10
	// _callPrice = 1
	// _warrantsPerToken = 4
	// _expireTime = 1546300800 (1/1/2019)
	return new RAZToken(_wallet, 10, 1, 4, 1546300800);
  }
